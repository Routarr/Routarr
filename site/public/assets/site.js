...
/* ------------------------------------------------------- departure board
   The board is complete without this: the first drawer is open in the markup
   and the clock reads as stopped. With it, the clock ticks in the reader's own
   time and a click on any departure opens the reasons it goes where it goes. */
(function () {
  var clocks = document.querySelectorAll('[data-clock]');
  if (clocks.length) {
    var lang = document.documentElement.lang || undefined;
    var tick = function () {
      // A board keeps a 24-hour clock whatever the language's habit.
      var now = new Date().toLocaleTimeString(lang, { hour: '2-digit', minute: '2-digit', second: '2-digit', hourCycle: 'h23' });
      Array.prototype.forEach.call(clocks, function (clock) { clock.textContent = now; });
    };
    tick();
    setInterval(tick, 1000);
  }

  Array.prototype.forEach.call(document.querySelectorAll('.dep-toggle'), function (button) {
    ...
    // The whole row answers a click, the button alone answers the keyboard.
